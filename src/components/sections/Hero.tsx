...
export async function Hero() {
  ...
  return (
    <section
      id="inicio"
      className="relative isolate overflow-hidden bg-background text-foreground dark:bg-ail-navy dark:text-white"
    >
      ...
      <div className="relative mx-auto w-[calc(100%-2rem)] max-w-[1440px] py-12 sm:w-[calc(100%-3rem)] sm:py-16 xl:max-w-[1520px] lg:py-20">
        <div className="grid w-full items-center gap-8 lg:grid-cols-[0.36fr_0.64fr] lg:gap-14">
          ...
          <div className="min-w-0 text-center lg:text-left">
            ...
            <h1 className="mt-3 font-display text-[1.9rem] font-bold leading-[1.12] text-ink sm:text-4xl lg:text-[2.6rem] dark:text-white">
              {headline.includes("conectan")
                ? headline
                : "Idiomas que conectan personas y oportunidades"}
            </h1>
            <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-muted sm:text-base lg:mx-0 dark:text-white/78">
              {hero.subheadline}
            </p>
            ...
            <ul className="mt-5 flex flex-wrap items-center justify-center gap-2 lg:justify-start">
              {heroLanguages.map((language) => (
                <li
                  key={language.name}
                  className="inline-flex items-center gap-2 rounded-full border border-navy/10 bg-white/70 px-3 py-1.5 text-sm font-medium text-ink dark:border-white/15 dark:bg-white/5 dark:text-white"
                >
